...
This means that there is a statistically significant difference in the median income of individuals in low residence houses than in high residence house

### 4. Kruskal-wallis H-test = one way ANOVA

A non-parametric statistical test used to determine whether two or more independent groups have different medians.

It's an extension of the Mann-Whitney U test for more than two groups and serves as a non-parametric alternative to the one-way analysis of variance (ANOVA) when the assumptions of ANOVA, particularly the assumption of normality, are not met.

$h_0$ medians of all groups are equal

$h_1$ at least one groups median differs from the rest

#### a. Test on income based on multiple categories of residences

```{r}
kruskal_data = quasi_binom_data
kruskal_test = kruskal.test(income~reside, data = kruskal_data)
kruskal_test
```

#### b. Interpretation

$h_0$ medians across the levels of income at all reside are not statistically different from each other

$h_1$ medians across the levels of income at all reside are statistically different from each other

because the p-value of `0.4127` is `> 0.05` the null hypothesis is not rejected.

This means that the there is not sufficient evidence to conclude significant difference in the median of income across the different residing people in the homes

### 5. Pearson Correlation = spearman rank correlation

Used to assess the relationship between two continuous variables but how they quantify and evaluate this relationship

#### a. Pearson

Assumes a linear relationship between variables and requires both variables to be normally distributed.
...
Doesn't assume linearity or normality and is suitable for ordinal or non-normally distributed data.

Measures the strength and direction of monotonic relationships between variables.

Similar to Pearson, but `it measures the monotonic relationship rather than the linear relationship`. It's robust against outliers

#### c. Assessment

```{r}
pearson_spearman = function(x){
  pearson_cor = cor(linear_regress_data$tollten,
                  linear_regress_data[,x], method = "pearson")
  spearman_cor = cor(linear_regress_data$tollten,
                  linear_regress_data[,x], method = "spearman")
  return (cat("the pearson corerelation for ", x, "is ", pearson_cor," and the spearman correlation is: ", spearman_cor, "\n"))
}
```

```{r}
continuous_data = c("equipten", "cardten", "wireten", "loglong", "logcard")
for (i in 1:length(continuous_data)){
  print(pearson_spearman(continuous_data[i]))
}
```

`cardten`, `wireten` , `logcard` and `loglong` have a strong linear correlation with `tollten` which because the pearson coefficient is significantly close to `0.5`

`logcard` has a weak positive correlation with `tollten` because their pearson correlation value is significantly close to `0`

## 7. Exploratory Analysis

### 1. PCA, Factor Analysis

Dimensionality reduction techniques used in statistics and data analysis, but they have different approaches and applications.

**PCA** Identifies a new set of uncorrelated variables (principal components) that are linear combinations of the original variables. These components are ordered by the amount of variance they explain

The main goal of PCA is to find a new set of variables, called principal components, which are linear combinations of the original variables.

**Factor Analysis** It assumes that observed variables are influenced by a smaller number of unobserved (latent) variables or factors. It explores the relationships between observed variables and factors.
...
#### b. Dataset with no na values and numeric
...
```{r}
# perform pca 
pca_analysis = prcomp(pca_data,
                      scale = TRUE) # avoid bias
summary(pca_analysis)
```

#### c. elements of the pca

```{r}
names(pca_analysis)
```

#### d. standard deviation of the components

```{r}
pca_analysis$sdev
```

#### e. Eigen vectors of pca

```{r}
pca_analysis$rotation
```

#### f. mean and standard deviation of the original variables

```{r}
pca_analysis$center # mean
pca_analysis$scale# standard deviation
```
...
color by age 1 young, 0 old

```{r}
pca_data$age = ifelse(pca_data$age < 35, 1, 0)
fviz_pca_biplot(pca_analysis,
                label = "var",
                habillage = pca_data$age)
```

### 2. Fixed effect and random effect models

**Fixed/ factor effect models** These models consider categorical variables (factors) with a fixed set of levels or categories

```{r}
factor_data = linear_regress_data
factor_data = factor_data %>% select_if(is.factor)
factor_data$age = linear_regress_data$age
```

```{r}
fact_eff_mod = lm(age~., factor_data)
summary(fact_eff_mod)
```

`churn` and `retirement` are the key factors that can help predict a person's age

**Random effect model** models are used when dealing with categorical variables or factors whose levels are sampled from a larger population.

```{r}
rem_data = linear_regress_data
rem_data = data.frame(sapply(rem_data[, c(2, 5, 7, 10)], as.factor))
rem_data$age = linear_regress_data$age 
```

```{r}
library(lme4)
rem_mod = lmer(age~(1|tenure ), data = rem_data) 
summary(rem_mod)
```
...
Suggests that the probability of missingness might depend on observed data but not on unobserved data after conditioning on other observed variables.

`mar and mnar` are performed by imputation and assessing the change due to imputation, for the linear regression data imputation causes no change in the spread of the data.

#### **b. CMAR**

It suggests that the missingness may depend on unobserved variables, making it more complex to handle compared to MAR

#### c. MNAR

MNAR implies that the missing values are systematically different from observed values and can introduce bias if not handled properly.

#### d. GEE regression

Useful when the correlation structure within the data needs to be considered, and it provides population-averaged estimates while accounting for within-subject correlation

```{r}
library(geepack)

# Fit GEE model
gee_model <- geeglm(age ~ ., data = factor_data,id = gender,
                    family =poisson())
summary(gee_model)
...
Gee predicts age using `retire` , `gender`, and `churn` with `custcat` as the key factors influencing `age`. This is by using gender as the grouping variable
